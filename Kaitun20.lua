...
function toTarget(p)
    ...
    local Tween = game:GetService("TweenService"):Create(PartTP, TweenInfo.new(Distance / 350, Enum.EasingStyle.Linear), {CFrame = p})
    Tween:Play()
    ...
    if ClosetV then
        ...
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", ClosetV)
        wait(.5)
        PartTP.CFrame = HumanoidRP.CFrame
    end
end
...
function FarmLevel()
	...
	if (MyLevel >= 20 and MyLevel <= 99) then
		if MyLevel >= 20 and MyLevel <= 99 then
			local CFrameMyMon = CFrame.new(-4721.88867, 843.874695, -1949.96643, 0.996191859, -0, -0.0871884301, 0, 1, -0, 0.0871884301, 0, 0.996191859)
