	...
	while wait() do
		local succ,err = pcall(function()
			if _G.KaitunConfig["Setting"]["Start Farm"] then
				...
				if FirstSea then
					if _G.KaitunConfig["Fruits"]["Config"]["Find Fruits"] then
						if FindFruit then
							FindFruit()
						end
					elseif _G.KaitunConfig["Item"]["World 1"]["Saber"] and MyLevel >= 200 and not HaveItem("Saber") and HaveMob("Saber Expert") then
						Saber()
						print("Saber")
				...
				elseif SecondSea then
					...
					elseif _G.KaitunConfig["Item"]["World 2"]["Acidum Rifle"] and HaveMob("Core") then
						AcidumRifle()
						print("Acidum Rifle")
				...
				elseif ThirdSea then
					...
					elseif _G.KaitunConfig["Item"]["World 3"]["Twin Hooks"] and MyLevel >= 1600 and HaveMob("Captain Elephant") then
						TwinHooks()
					...
					elseif _G.KaitunConfig["Sword"]["World 3"]["Yama"] and MyLevel >= 2500 and game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("EliteHunter","Progress") >= 30 and not HaveItem("Yama") then
						Yama()
					elseif _G.KaitunConfig["Sword"]["World 3"]["Tushita"] and MyLevel >= 2000 and ThirdWorld and game.Workspace.Enemies:FindFirstChild("rip_indra True Form [Lv. 5000] [Raid Boss]") or game.ReplicatedStorage:FindFirstChild("rip_indra True Form [Lv. 5000] [Raid Boss]") then
						Tushita()
					else
						FarmLevel()
					end
			...
			if string.find(v.Name, "Fruit") then
				repeat task.wait()
					if (v.Handle.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 1500 then
						...
						toTarget(v.Handle.CFrame * CFrame.new(0,0,10))
					end	
					EquipMelee()
				until v.Parent == game.Players.LocalPlayer.Character or _G.KaitunConfig["Fruits"]["Config"]["Find Fruits"] == false or FindFruit == false
			end
		end
	end
end
